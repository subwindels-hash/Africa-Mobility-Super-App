...
export const PHASES: Record<number, { name: string; enable: string[] }> = {
  1: { name: 'Phase 1 — Nigeria core', enable: ['transportation', 'logistics', 'wallet', 'escrow', 'whatsapp_ai'] },
  2: { name: 'Phase 2 — Travel & services', enable: ['travel', 'hotels', 'corporate_services', 'roadside'] },
  3: { name: 'Phase 3 — Security marketplace', enable: ['security'] },
  4: { name: 'Phase 4 — Aviation marketplace', enable: ['aviation'] },
...
export function seedFams(engine: FamsEngine, activePhase = 4): void {
  ...
  // Edo state: helicopter/aviation off (state control — covers Benin City)
  ...
  for (const f of ['ai.dynamic_pricing', 'whatsapp.ai', 'comms.video', 'wallet', 'escrow']) {
